...
// --- Helper function to read current form values --- 
function getFormConfigs(): ConfigType {
    return {
...
document.querySelector('#llmProvider').addEventListener('change', (event) => {
    ...
    // Re-enable the test button immediately when the provider changes
...
document.querySelectorAll('#optionsForm select').forEach((node) => {
    node.addEventListener('change', (event) => {
        // Ignore the provider select, as its specific listener handles the button state
        if ((event.target as HTMLElement)?.id === 'llmProvider') {
            return;
        }
        document.querySelector<HTMLButtonElement>('#optionsForm button.test').disabled = true
    })
...
document.querySelector('#optionsForm button.test').addEventListener('click', async (event) => {
    ...
    const testResultDiv = document.querySelector('#optionsForm #testResult');
    const testResultMessage = document.querySelector('#optionsForm #testResult .message');
    testResultDiv.classList.remove('ok', 'ko');
    testResultMessage.innerHTML = '...'; // Indicate testing in progress

    try {
        // Construct config directly from current form values for the test
        const configs = getFormConfigs(); // Use the helper function

        // Create LlmService using the current form values
        ...
        // Success
        testResultDiv.classList.add('ok');
        testResultMessage.innerHTML = messenger.i18n.getMessage('options.testSuccessful');

    } catch (error) {
        // Failure
        testResultDiv.classList.add('ko');
        testResultMessage.innerHTML = error instanceof Error ? error.message : 'Connection test failed';
        // Log the full error for debugging
        console.error('Connection test failed:', error);
    } finally {
        // Always remove testing state and re-enable button (unless successful)
        testButton.classList.remove('testing');
        if (!testResultDiv.classList.contains('ok')) {
            // Re-enable button only if the test failed, allowing user to retry after fixing config
            // If successful, button remains disabled until config changes again.
